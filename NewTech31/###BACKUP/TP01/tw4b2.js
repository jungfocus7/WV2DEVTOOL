...
// ()/*+-0123456789.
{
    ...
    const fn_checkCharacters = () => {
        ...
        switch (_cch) {
            case '(': {
                br = fn_indexOfDual(
                    '(/*+-', true, _pch,
                    '(-0123456789', false, _nch
                );
                if (br === false) {
                    fn_err(`[${_pch},${_cch},${_nch}]; Invalid tokens.`);
                } else {
                    _wg++;
                }
                break;
            ...
            case '-': {
                br = (
                    ...
                    (
                        // fn_includes(')0123456789', true, _pch) &&
                        fn_includes(')0123456789', false, _pch) &&
                        fn_equals('-', false, _nch)
                    )
                );
                if (br === false) {
                    fn_err(`[${_pch},${_cch},${_nch}]; Invalid tokens.`);
            ...
            default: {
                if (fn_includes('0123456789', false, _cch)) {
                    br = fn_indexOfDual(
                        '(/*+-0123456789.', true, _pch,
                        ')/*+-0123456789.', true, _nch
                    );
                    if (br === false) {
                        fn_err(`[${_pch},${_cch},${_nch}]; Invalid tokens.`);
                    }
                } else if (fn_equals('.', false, _cch)) {
                    br = fn_indexOfDual(
                        '0123456789', false, _pch,
                        '0123456789', false, _nch
                    );
    ...
    const fn_checkNumChar = () => {
        ...
        } else if (fn_includes('0123456789', false, _cch)) {
            if (fn_includes('(/*+-0123456789', true, _pch)) {
                br = fn_includes(')/*+-0123456789.', true, _nch);
            } else if (fn_equals('.', false, _pch)) {
                br = fn_includes(')/*+-0123456789', true, _nch);
            }
        } else if (fn_equals('.', false, _cch)) {
            if (_buff.indexOf('.') === -1) {
     ...
     * @param {string} tk
     */
    const fn_isNumToken = (tk) => {
        const regNumVal = /^-?(\d+\.\d*|\.\d+|\d+)$/;
        return regNumVal.test(tk);
    };
    ...
    const fn_checkPushOperator = (op) => {
        if (fn_equals('(', false, op)) {
            _gmks.push(_vals.length);
        }
        _ops.push(op);
    };
    ...
    const fn_calcNextApply = () => {
        ...
        if (fn_equals('(', false, cop)) {
            fn_err('Unbalanced parentheses: unmatched opening parenthesis.');
        }
        ...
        // if ((undefined === rv) || (undefined === lv)) {
        //     fn_err(`Insufficient operands for operator "${cop}".`);
        // }

        if (
            (fn_equals('/', false, cop) || fn_equals('%', false, cop)) &&
            (rv === 0)
        ) {
            fn_err('Division by zero is not allowed.');
        }
    ...
    const fn_checkMiddleWave = () => {
        ...
        if (_ops.length > 0) {
            ...
            if (_vals.length <= mark) {
                fn_err('Empty parentheses group: no value produced.');
            }
        } else {
            fn_err('Unbalanced parentheses: unmatched closing parenthesis.');
        }
    };
    ...
    const fn_calcEntry = (txt) => {
        ...
        fn_tokenize();
        if (_tokens.length === 0) {
            fn_err('Empty expression.');
        }

        for (let tk of _tokens) {
            // console.log(tk);
            if (fn_equals('(', false, tk)) {
                fn_checkPushOperator(tk);
            } else if (fn_equals(')', false, tk)) {
                fn_checkMiddleWave();
            } else if (fn_isNumToken(tk)) {
                fn_checkPushValue(tk);
            } else if ('/*%+-'.indexOf(tk) > -1) {
                fn_checkStackOutWave(tk);
            } else {
                fn_err('Invalid token verified.');
            }
        }
        ...
        console.log(`>>>1 ${_txt} = ${_rdv}`);
        // console.log(`>>>2 ${_txt} = ${globalThis.eval(_txt)}`);
        return _rdv;
    };
    //#endregion
