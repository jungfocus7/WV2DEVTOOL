...
(() => {
    _btnStart.addEventListener('click', (me) => {
        ...
        for (const ge of _geCont.children) {
            const rci = _rcia.at(j);
            // console.log(ge, j, rci);
            console.log(rci);
            // rci.initTarget(ge);
        ...
        //     { color: 'blue', transform: 'scale(1.2)' },
        // ];

        // // 3. options 정의
        // const myOptions = {
        //     duration: 2000, // 2초
        //     easing: 'cubic-bezier(0.25, 0.1, 0.25, 1)', // 커스텀 이징 함수
        //     fill: 'forwards' // 최종 상태 유지
        // };

        console.log(_rcia);

        // 4. animate 메서드 호출
        // const animation = _rcia.at(0).get_tge().animate(myKeyframes, myOptions);
    });
